...
def ayylama2():
    camera.start_recording(output2, format='mjpeg', splitter_port=2)
    try:
        address = ('192.168.1.159', 8080)
        server = StreamingServer(address, StreamingHandler)
        server.serve_forever()
    finally:
        camera.stop_recording()
    """stream = BytesIO()
    camera.start_recording(stream, format='h264', quality=23, splitter_port=2)
    try:
